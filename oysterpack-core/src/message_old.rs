...
impl SealedEnvelope {
    ...
    /// decodes the stream and returns 
    ...
    pub fn open(self, key: &box_::PrecomputedKey) -> Result<OpenEnvelope, Error> {
        box_::open_precomputed(&self.msg, &self.nonce, key).map(|msg| OpenEnvelope {
...
impl fmt::Display for Addresses {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        write!(f,"[{}] -> [{}]", base58::encode(&self.sender.0),base58::encode(&self.recipient.0))
    }
}
...
mod test {
    ...
    use super::{Addresses, MessageType, OpenEnvelope, SealedEnvelope, base58};
